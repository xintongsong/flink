...
public class ActiveResourceManagerTest {
	...
	@Test
	public void testPendingWorkerCounter() {
		final WorkerResourceSpec spec1 = new WorkerResourceSpec(1.0, 1, 1, 1, 1);
		final WorkerResourceSpec spec2 = new WorkerResourceSpec(2.0, 1, 1, 1, 1);
		...
		try {
			counter.decreaseAndGet(spec2);
			fail("Should have failed calling `decreaseAndGet` on zero.");
		} catch (Throwable t) {
			// expected
		}
	}
