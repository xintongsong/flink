...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
	...
	private ApplicationReport startAppMaster(
		...
		final ShipFileUtils shipFileUtils = new ShipFileUtils(flinkConfiguration, yarnConfiguration, appId);
		...
		List<String> systemClassPaths = shipFileUtils.uploadAndRegisterFiles(
			systemShipFiles,
			paths,
			localResources,
			Path.CUR_DIR,
		...
		final List<String> userClassPaths = shipFileUtils.uploadAndRegisterFiles(
			userJarFiles,
			paths,
			localResources,
		...
		if (jobGraph != null) {
			...
			try {
				...
				Path pathFromYarnURL = shipFileUtils.setupSingleLocalResource(
						jobGraphFilename,
		...
		try {
			...
			Path remotePathConf = shipFileUtils.setupSingleLocalResource(
				flinkConfigKey,
	...
	static class YarnDeploymentException extends RuntimeException {
